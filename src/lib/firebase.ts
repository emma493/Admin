...
/**
 * Subscribe in real-time to all video ads in Firestore
 */
export function subscribeToAds(
  onData: (ads: AdDocument[]) => void,
  onError?: (err: Error) => void
) {
  ...
  return onSnapshot(
    adsRef,
    (snapshot) => {
      const items: AdDocument[] = snapshot.docs.map((docSnap) => {
        const data = docSnap.data();
        return {
          id: docSnap.id,
    ...
    (err) => {
      handleFirestoreError(err, OperationType.LIST, ADS_COLLECTION);
      if (onError) onError(err);
    }
  );
}
...
/**
 * Save or update an ad document in Firestore
 */
export async function saveAdDoc(adData: Omit<AdDocument, 'id'> & { id?: string }): Promise<string> {
  try {
    const isUpdate = !!adData.id;
    const docRef = isUpdate
      ? doc(db, ADS_COLLECTION, adData.id!)
      : doc(collection(db, ADS_COLLECTION));
    ...
    handleFirestoreError(err, OperationType.WRITE, ADS_COLLECTION);
    throw err;
  }
}
 ...
 */
export async function deleteAdDoc(id: string): Promise<void> {
  try {
    const docRef = doc(db, ADS_COLLECTION, id);
    await deleteDoc(docRef);
  } catch (err) {
    handleFirestoreError(err, OperationType.DELETE, `${ADS_COLLECTION}/${id}`);
    throw err;
  }
}

/**
 * Toggle ad is_active status
 */
export async function toggleAdStatus(id: string, currentIsActive: boolean): Promise<void> {
  try {
    const docRef = doc(db, ADS_COLLECTION, id);
    await updateDoc(docRef, { is_active: !currentIsActive });
  } catch (err) {
    handleFirestoreError(err, OperationType.UPDATE, `${ADS_COLLECTION}/${id}`);
    throw err;
  }
}

/**
 * Perform atomic increment on the views field inside the ads document in Firestore.
 */
export async function incrementAdViews(adIdOrUrl: string): Promise<void> {
  if (!adIdOrUrl) return;
  try {
    const directRef = doc(db, ADS_COLLECTION, adIdOrUrl);
    const directSnap = await getDoc(directRef);
    if (directSnap.exists()) {
      await updateDoc(directRef, { views: increment(1) });
      return;
    ...
    const adsRef = collection(db, ADS_COLLECTION);
    const qDirect = query(adsRef, where('direct_url', '==', adIdOrUrl));
    const directQuerySnap = await getDocs(qDirect);
    if (!directQuerySnap.empty) {
      const matchDoc = directQuerySnap.docs[0];
      await updateDoc(doc(db, ADS_COLLECTION, matchDoc.id), { views: increment(1) });
      return;
    }

    const qPage = query(adsRef, where('page_url', '==', adIdOrUrl));
    const pageQuerySnap = await getDocs(qPage);
    if (!pageQuerySnap.empty) {
      const matchDoc = pageQuerySnap.docs[0];
      await updateDoc(doc(db, ADS_COLLECTION, matchDoc.id), { views: increment(1) });
    ...
    console.warn('Error incrementing ad views:', adIdOrUrl, err);
  }
}

/**
 * Fetches an active video ad stream directly and increments its view by 1
 */
export async function fetchNextAdAndTrackView(excludeIds: string[] = []): Promise<AdDocument | null> {
  try {
    const adsRef = collection(db, ADS_COLLECTION);
    ...
    await updateDoc(doc(db, ADS_COLLECTION, chosenId), { views: increment(1) });
    ...
    console.error('Error in fetchNextAdAndTrackView:', err);
    return null;
  }
}

/**
 * Real-time listener for Total Ad Views
 */
export function subscribeToTotalAdViews(
  onData: (totalViews: number) => void,
  onError?: (err: Error) => void
) {
  const adsRef = collection(db, ADS_COLLECTION);
...
/**
 * Real-time listener for Ad Views (24H)
 */
export function subscribeTo24hAdViews(
  onData: (views24h: number) => void,
  onError?: (err: Error) => void
) {
  const twentyFourHoursAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
  ...
  const q = query(
    eventsRef,
    where('event_type', 'in', ['ad_view', 'video_ad_view']),
    where('timestamp', '>=', twentyFourHoursAgo)
  );
  ...
  return onSnapshot(
    ...
    (err) => {
      console.warn('Notice: 24h ad views falling back to event query:', err);
      const fallbackQuery = query(eventsRef, where('event_type', 'in', ['ad_view', 'video_ad_view']));
      return onSnapshot(
        fallbackQuery,
        (fallbackSnap) => {
          let count = 0;
